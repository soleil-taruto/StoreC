	...
	public class Test0011
	{
		public void Test01()
		{
			...
			Enumerable.Range(1, 3)
				...
				.Test0011_ForEach(v => Console.WriteLine("2 : " + v[0])) // { -1, -2, -3 } にならない。-- { 1, 2, 3 } のまま。
			...
			Enumerable.Range(1, 3)
				.Select(v => new int[] { v })
				.Test0011_ForEach(v => Console.WriteLine("1 : " + v[0]))
				.Test0011_ForEach(v => { v[0] *= -1; Console.WriteLine("2 : " + v[0]); }) // 3 : の前に表示されて { -1, -2, -3 } と表示されるけど ...
				.Test0011_ForEach(v => Console.WriteLine("3 : " + v[0])) // { -1, -2, -3 } にならない。-- { 1, 2, 3 } のまま。
				.DistinctOrderBy((a, b) => a[0] - b[0])
